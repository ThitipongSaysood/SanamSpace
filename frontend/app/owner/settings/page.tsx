      ...
      <div className="space-y-6 lg:col-span-2">
        ...
        <section className="space-y-3 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
          ...
          <label className="flex items-start gap-3">
            <input
              type="checkbox"
              checked={form.pointsEnabled === true}
              onChange={(e) => set("pointsEnabled", e.target.checked)}
              className="mt-0.5 size-4 accent-[var(--brand-primary)]"
            />
            <span className="text-sm">
              ให้คะแนนลูกค้าเมื่อจองและชำระเงินครบ
              <span className="mt-0.5 block text-xs text-muted-foreground">
                ให้ตอนจ่ายเงินครบ ไม่ใช่ตอนกดจอง · ยกเลิกแล้วระบบหักคะแนนคืนอัตโนมัติ
              </span>
            </span>
          </label>
          ...
          {form.pointsEnabled && (
            <div className="space-y-3">
              <div className="max-w-xs space-y-1.5">
                <Label htmlFor="points-rate">จองครบ 1 ครั้ง ได้กี่คะแนน</Label>
                <Input
                  id="points-rate"
                  type="number"
                  min={0}
                  value={form.pointsPerBooking ?? 10}
                  onChange={(e) => set("pointsPerBooking", Number(e.target.value))}
                />
              ...
              <div className="space-y-1.5">
                <Label>ระดับสมาชิก (ใช้คะแนนสะสมตลอดชีพ)</Label>
                <div className="grid gap-2 sm:grid-cols-3">
                  {Object.entries(form.tierThresholds ?? { Silver: 0, Gold: 500, Platinum: 2000 }).map(
                    ([tier, need]) => (
                ...
                {/* The two settings are keyed by the same tier names, so a
                    mismatch is a discount nobody can ever reach. */}
                <p className="text-xs text-muted-foreground">
                  ชื่อระดับต้องตรงกับที่ตั้งไว้ใน “ส่วนลดสมาชิก” ไม่งั้นลูกค้าไต่ถึงระดับแล้วแต่ไม่ได้ส่วนลด
                </p>
              </div>
            </div>
          )}
