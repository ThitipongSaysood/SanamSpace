...
async function ownerHeaders(request: APIRequestContext) {
  ...
  return { Authorization: `Bearer ${(await login.json()).token}`, "X-Venue-Slug": VENUE };
}
 ...
 * creates them.
 */
test("a walk-in and their LINE account can be folded into one customer", async ({ page, request }) => {
  const headers = await ownerHeaders(request);
  const courts = await (await request.get(`${API}/owner/courts`, { headers })).json();
  ...
  const booking = await request.post(`${API}/owner/bookings`, {
    headers,
    data: {
      courtId, date, start: "07:00", end: "08:00",
      customerName: "คุณซ้ำ (หน้าร้าน)", customerPhone: phone, status: "confirmed",
    },
  });
  ...
  const login = await request.post(`${API}/auth/line/login`, {
    data: { organizationSlug: VENUE, lineUserId: `Umerge${suffix}`, displayName: "คุณซ้ำ (LINE)" },
  });
  ...
  const profile = await request.put(`${API}/auth/me`, {
    headers: { Authorization: `Bearer ${customerToken}`, "X-Venue-Slug": VENUE },
    data: { phone },
  });
  ...
  await group.getByRole("button", { name: "เก็บใบนี้ รวมที่เหลือเข้ามา" }).first().click();
  const sheet = page.getByRole("dialog", { name: "ยืนยันการรวมลูกค้า" });
  await expect(sheet).toBeVisible();
  await sheet.getByRole("button", { name: "รวมเข้าใบที่เก็บ" }).first().click();
  ...
  // without releasing it the second run finds 07:00 taken and fails.
  await request.post(`${API}/owner/bookings/${bookingId}/cancel`, { headers, failOnStatusCode: false });
});
