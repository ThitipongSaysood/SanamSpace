...
/** Centered modal dialog shared by the Admin and Owner portals. */
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" role="dialog" aria-modal="true">
      ...
      <div className={`relative w-full ${width} rounded-2xl bg-white shadow-xl`}>
        ...
        {/* Wraps: the invoice dialog carries four actions, which overflow a
            narrow modal if they are forced onto one line. */}
        {footer && (
          <div className="flex flex-wrap justify-end gap-2 border-t border-black/5 px-5 py-3.5">{footer}</div>
        )}
      </div>
    </div>
  );
