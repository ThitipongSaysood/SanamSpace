...
def build(scan_path, out_path):
    ...
    rows = []
    for it in items:
        st, ev = res.get(it["id"], ("ยังไม่ตรวจ", ""))
        rows.append({**it, "status": st, "evidence": ev})
    ...
    wb.save(out_path)
    return {"domain": domain, "auto_checked": len(auto), "failed": len(fails),
            "by_risk": {k: sum(1 for f in fails if f["risk"] == k) for k in RISK_ORDER}}


if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("scan_json")
    ap.add_argument("--out", default="Security_Report.xlsx")
    a = ap.parse_args()
    print(json.dumps(build(a.scan_json, a.out), ensure_ascii=False, indent=1))
